...
export default function BookingModal({ handleClose, modalState }) {
  ...
  const getHotelDetails = async (city, hotelId) => {
    try {
      const response = await axios.get(`https://feastfinder-backend-nwph.onrender.com/api/restaurants/${city}/${hotelId}`);
    ...
    if (userId && userName && userEmail && id && selectedDate && selectedTime && selectedSeats) {
      try {
        const response = await axios.post(
          "https://feastfinder-backend-nwph.onrender.com/api/create-booking",
          {
            userId,
            userName,
            userEmail,
  ...
  const handleReviewSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        "https://feastfinder-backend-nwph.onrender.com/api/submit-review",
        {
          hotelId: id,
          userId,
          name,
  ...
  const handleAdminReply = async (reviewId) => {
    ...
    try {
      const response = await axios.post(
        "https://feastfinder-backend-nwph.onrender.com/api/admin/reply",
        {
          reviewId,
          adminReply: reply,
        }
